  ...
  var CATEGORIES = {
    ...
    idiomEveryday: { name:"اصطلاحات روزمره", words: [
    ...
    idiomFeelings: { name:"اصطلاحات احساسات", words: [
    ...
    idiomEffort: { name:"اصطلاحات کار و تلاش", words: [
    ...
    idiomBehavior: { name:"اصطلاحات رفتار و آداب", words: [
  ...
  CATEGORIES.a2everyday = { name:"روزمره (A2)", words: [
  ...
  function updateTileProgress(){
    ...
    for (var key in homeMap){
      ...
      if (el && CATEGORIES[key]){
        ...
        el.textContent = total + " لغت • " + toPersianDigits(pct) + "٪ بلد";
      }
    }
    ...
    for (var skey in subMap){
      ...
      if (sel && CATEGORIES[skey]){
        ...
        sel.textContent = stotal + " اصطلاح • " + toPersianDigits(spct) + "٪";
      }
    }
  }
  ...
  function updateStreak(){
    try{
      ...
      if (profileStreakEl) profileStreakEl.textContent = streak || 1;
    }catch(err){
  ...
  nextBtn.addEventListener("click", function(){
    if (idx < WORDS.length - 1){ idx++; render(); }
      ...
      document.getElementById("celebrateWordCount").textContent = WORDS.length;
  ...
  function triggerProfileAnimations(){
    if (!profileTargetsRead){
      ...
      if (se){
        var s = parseInt(se.textContent, 10);
  ...
  (function runLoading(){
    ...
    function tick(){
      ...
      if (pct >= 100){
        ...
        setTimeout(function(){ showScreen("category"); }, 250);
        return;
      }
  ...
  backToCategoryBtn.addEventListener("click", function(){
    showScreen(currentCategoryKey.indexOf("idiom") === 0 ? "idioms" : "category");
  });
